...
import ExperimentRule from "./ExperimentRule";
...
export default class RuleService {
    ...
    createRule(data : {
        ...
        switch (data.output.suffix) {
            ...
            case "experiment":
                return new ExperimentRule(data.output.prefix,data.output.suffix, data.match.event, data.match?.css, data.match?.xpath, data.match?.code, data.match?.key, data?.context?.url, data?.context?.css, data?.context?.xpath, data.description);
            case "attributeValue":
